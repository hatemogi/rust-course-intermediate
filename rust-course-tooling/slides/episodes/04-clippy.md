...
# Clippy 판단과<br>첫 검증 실습

lint를 검토하고 세 검사 명령을 하나의 흐름으로 연결합니다.

---
level: 2
---

# Clippy는 컴파일되는 코드에서 단서를 찾습니다
...
<div class="takeaway">Clippy 제안은 강한 단서이지만 프로그램의 요구 사항은 아닙니다.</div>
...
<table class="compare">
  <tbody>
    <tr><td><code>--all-targets</code></td><td>예제, 테스트와 벤치마크까지 검사</td></tr>
    <tr><td><code>--locked</code></td><td>잠금 파일을 바꿔야 하면 실패</td></tr>
    <tr><td><code>--</code></td><td>뒤의 옵션을 rustc와 Clippy에 전달</td></tr>
...
# 코드는 의도를 바로 드러낼수록 읽기 쉽습니다

<<< ../../examples/02_clippy.rs rust {1-3|2}{lines:true}

`Iterator::any`는 조건에 맞는 값이 하나라도 있는지를 그대로 표현합니다.

<div class="takeaway">제안된 문법만 보지 말고 lint가 경고하는 위험이나 낭비를 먼저 설명합니다.</div>

---
level: 2
---

# lint 하나를 네 단계로 판단합니다

<ol class="step-list">
  <li>경고하는 실제 위험이나 낭비를 설명합니다.</li>
...
AI가 이런 수정을 제안했다면 원래 lint를 다시 실행해 하나씩 판단합니다.
...
<div class="tool-flow">
  ...
  <div><strong>lint</strong>선택한 범위에 의심스러운 표현이 남았는가?</div>
</div>
...
<table class="compare">
  ...
  <tbody>
    ...
    <tr><td><code>cargo clippy</code></td><td>선택한 lint가 남지 않음</td><td>모든 입력에서 올바름</td></tr>
  </tbody>
</table>
...
# 컴파일과 lint를 통과한 코드가<br>요구 사항대로 동작하는지는 어떻게 확인할까요?

전체 과정에서는 편집기 자동화, 의존성, 문서, 테스트와 측정을 하나의 검증 흐름으로
연결합니다.
