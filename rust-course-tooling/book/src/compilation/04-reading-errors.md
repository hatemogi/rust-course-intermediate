...
Rust 컴파일 오류에는 단순히 실패했다는 사실뿐만 아니라 컴파일러가 확인한 위치,
관련된 값과 가능한 수정 방법이 함께 표시됩니다. AI에게 바로 수정을 맡기기보다는
오류 메시지가 알려 주는 내용을 먼저 이해하는 편이 좋습니다.
...
4. 비고에서 앞선 이동, 빌림이나 타입 결정 위치를 찾습니다.
5. 도움말의 제안이 프로그램의 의도와 맞는지 판단합니다.

<figure>
  <img src="../images/tooling/rustc-diagnostic-anatomy.svg"
       alt="E0382 오류에서 오류 코드부터 위치, 문제가 된 값, 비고, 도움말까지 차례로 읽는 모습">
  <figcaption>오류 메시지는 위에서 아래로 읽되, 제안은 프로그램의 의도와 맞는지 따로 판단합니다.</figcaption>
</figure>

## 이동한 값을 다시 사용한 예제

다음 파일은 컴파일 오류를 살펴보기 위해 일부러 잘못된 코드를 넣었습니다.

```rust,compile_fail
{{#include ../../../examples/compile_fail/moved_value.rs}}
```

`rust-course-tooling` 디렉터리에서 다음 명령을 실행하면 이 파일만 컴파일해 볼 수
있습니다.

```bash
rustc examples/compile_fail/moved_value.rs
```

이 예제는 컴파일에 실패하도록 작성했으므로 실행 파일이 만들어지지 않고 명령도 실패
상태로 끝납니다. 이어서 출력되는 `E0382` 오류를 읽는 것이 이 예제의 목적입니다.

`print_message`가 `String`을 값으로 받으므로 `message`의 소유권이 함수로 이동합니다.
뒤의 `println!`에서 같은 값을 다시 사용하면 컴파일러는 `E0382`를 보고합니다.

다음 명령으로 오류 코드의 자세한 설명을 읽을 수 있습니다.
