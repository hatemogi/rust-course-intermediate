...
`Cargo.toml`에는 프로젝트가 허용하는 의존성 범위를 사람이 작성합니다.
`Cargo.lock`에는 Cargo가 그 범위 안에서 실제로 선택한 직접·간접 의존성의 정확한
버전과 출처가 기록됩니다. `Cargo.lock`은 Cargo가 관리하므로 직접 편집하지
않습니다.
...
첫 번째 명령은 `[dependencies]`에 `serde`를 추가하고 `derive` feature를 켭니다.
...
비슷한 다른 패키지를 추가하지 않았는지, 불필요한 feature가 켜지지 않았는지
확인해야 합니다.
...
첫 번째 명령은 갱신할 수 있는 의존성을 모두 다시 선택합니다. 변경 범위를 줄이고
싶다면 두 번째 명령처럼 패키지를 지정합니다. 직접 의존성 하나를 갱신해도 그
...
`Cargo.toml`과 `Cargo.lock`의 역할은 [Cargo 공식 안내서](https://doc.rust-lang.org/cargo/guide/cargo-toml-vs-cargo-lock.html)에서
더 자세히 설명합니다.
