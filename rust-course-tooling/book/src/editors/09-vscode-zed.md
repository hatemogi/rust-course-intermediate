# VS Code와 Zed에서 rustfmt와 Clippy 사용하기
...
먼저 프로젝트가 사용하는 도구 체인에 필요한 구성 요소가 있는지 확인합니다.

```bash
rustup component add rustfmt clippy
```

편집기에서는 Cargo 프로젝트의 루트, 즉 `Cargo.toml`이 있는 디렉터리를 여세요.
...
맞출 수 있습니다. Clippy가 제안한 빠른 수정은 적용하기 전에 코드의 동작과 공개
API가 그대로인지 확인하세요.
...
- `formatter`를 `language_server`로 지정하면 Rust 파일은 rust-analyzer를 거쳐
  rustfmt로 정리됩니다.
- `check.command`를 `clippy`로 지정하면 rust-analyzer의 진단에 Clippy 결과가
...
이 강의 프로젝트에서는 두 명령뿐 아니라 테스트, 문서 테스트, 벤치마크 컴파일과
mdBook 빌드까지 `make check`로 확인합니다.
