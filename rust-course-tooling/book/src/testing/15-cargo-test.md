...
`cargo test`는 프로젝트의 테스트 코드를 컴파일한 뒤 `#[test]`가 붙은 함수를 찾아
실행합니다.
...
```rust
{{#include ../../../examples/05_testing.rs}}
```
...
`cargo test` 뒤에 문자열을 쓰면 테스트 이름에 그 문자열이 들어간 테스트만
실행합니다.
...
테스트 프로그램에 옵션을 전달할 때는 `--`로 Cargo 옵션과 구분합니다. 테스트가
출력한 내용을 숨기지 않으려면 다음과 같이 실행합니다.
